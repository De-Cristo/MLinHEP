...
parser.add_argument("-s", "--eventSea", dest="sea", type=str, default='two', help="Low Coupling value (default = two)")
parser.add_argument("-t", "--eventTarget",dest="target", type=str, default='four',help="High Coupling value (default = four")
parser.add_argument("-b", "--BackgroundType",dest="bkg", type=str, default='TT_',help="Type of the pending reweighting background (default = TT_")
...
bkg_name = args.bkg
os.system('mkdir -p RwT_Variables_Plots_{0}_{1}'.format(args.bkg,args.target))
plot_path = os.path.abspath('./RwT_Variables_Plots_{0}_{1}'.format(args.bkg,args.target))
...
for _y in years:
    for _file in os.listdir(path+'/'+_y+'/'):
        sample_pool.append(path+'/'+_y+'/'+_file)

sea_events_key = []
target_events_key= []

for _sample in sample_pool:
    if '{}_btag'.format(args.sea) in _sample and bkg_name in _sample:
        sea_events_key.append(_sample)
    elif '{}_btag'.format(args.target) in _sample and bkg_name in _sample:
        target_events_key.append(_sample)
    else:
        continue
        
rdf_dict['sea'] = R.RDataFrame('Events',sea_events_key)
rdf_dict['target'] = R.RDataFrame('Events',target_events_key)
